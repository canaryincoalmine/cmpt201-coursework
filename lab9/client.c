...
TCP, because it uses SOCK_STREAM.
3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? from STDIN, using read(STDIN_FILENO,
buf, BUF_SIZE).
4. How does the client program end? How can you tell that in the code?
it ends when the end of the file is reached, or when an error occurs. close(sfd)
closes the socket and exit(EXIT_SUCCESS) ends the client program.
*/
