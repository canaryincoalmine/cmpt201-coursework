...
1. How is the client sending data to the server? What protocol? The client uses
a TCP socket, so the TCP protocol.
2. What data is the client sending to the server? It's sending the static
strings (like "Hello" and "Apple") as 1024 byte strings. Understanding the
Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
The argument is a pointer to a struct containing: a shared run variable so it
can be shut down from the main thread, a pointer to the message list, and the
mutex lock for the list.
2. How are received messages stored? They're stored in a linked list.
3. What does `main()` do with the received messages? It prints each message and
then frees it. It also makes sure they were all received correctly.
4. How are threads used in this sample code? The acceptor threads create client
threads for clients they accept, and the client threads read and store messages.
*/
  ...
  while (aargs->run) {
    if (num_clients < MAX_CLIENTS) {
      ...
      if (cfd == -1) {
        ...
        pthread_create(&threads[num_clients++], NULL, (void *)&client_args,
                       run_client);
      }
    }
  }
